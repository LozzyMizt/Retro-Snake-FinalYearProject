    ...
    while True:  # main game loop
        ...
        if stalling and not directionList:
            ...
            if onlyDirection and onlyDirection == lastDirection:
                ...
                elif (not findNewHead(direction, wormCoords) in wormCoords) or (
                        findNewHead(direction, wormCoords) in wallCoords):
        ...
        '''
        if stalling:
            if AreaIsTooSmall(CELLWIDTH, nextHead, wormCoords,
                              lastWall):  # return true if the area going in is too small
                lastWall = 0
                directionList = findNextDirection(wormCoords, directionList[0], 0)
...
def checkSmartTurn(worm, listOfNo, direction1, direction2):
    if direction1 == UP or direction1 == DOWN:
        if direction2 == RIGHT:
            if {'x': worm[HEAD]['x'] + 3, 'y': worm[HEAD]['y']} in listOfNo and (
            not {'x': worm[HEAD]['x'] + 2, 'y': worm[HEAD]['y']} in listOfNo):
                return True
            else:
                return False
        if direction2 == LEFT:
    ...
    if direction1 == LEFT or direction1 == RIGHT:
        if direction2 == UP:
            if {'x': worm[HEAD]['x'], 'y': worm[HEAD]['y'] - 3} in listOfNo and (
            not {'x': worm[HEAD]['x'], 'y': worm[HEAD]['y'] - 2} in listOfNo):
                return True
            else:
                return False
        if direction2 == DOWN:
